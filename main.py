...
def last_3_seasons(player, last_season_less_3, player_color):
    ...
    elif player_name == 'Kobe Bryant':
        ax.annotate(f'{player_name} ({player["mp"][0]} mins played)', xy=(player.index[0], player['pts'][0]), xytext=(player.index[0] + 0.2, player['pts'][0]), color = player_color)
    else:
