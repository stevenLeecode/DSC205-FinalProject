...
ax.annotate(f'Michael Jordan ({mj["mp"][0]} mins played)', xy=(mj.index[0], mj['pts'][0]), xytext=(mj.index[0] + 0.1, mj['pts'][0] - 155), color = 'red')
...
def plot_reg_plot(player, colour):
  player_name = player['player'][0]
  fig = plt.figure(figsize = (16, 8))
  sns.regplot(x = player['season'], y = player['pts'], color = colour).set_title(f'{player_name} Total Points Per Season')
